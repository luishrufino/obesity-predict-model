...
class FeatureEngineering(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        required_cols = ['Age', 'Height', 'Weight', 'FCVC', 'NCP', 'FAF', 'TUE', 'MTRANS', 'Age']
        ...
        X['AgeGroup'] = (X['Age'] // 10).astype(int)
...
class MinMaxScalerFeatures(BaseEstimator, TransformerMixin):
    def __init__(self, min_max_col=None):
        if min_max_col is None:
            self.min_max_col = ['Age', 'Height', 'Weight', 'IMC', 'ActivityBalance', 'HealthyMealRatio', 'AgeGroup']
        else:
